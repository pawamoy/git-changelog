...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `git_changelog`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in git_changelog.__all__ or not hasattr(git_changelog, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in git_changelog.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("git_changelog")
